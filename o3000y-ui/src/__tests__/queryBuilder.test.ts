...
describe('buildTimeSeriesSQL', () => {
  ...
  it('builds avg query with measure', () => {
    ...
    const sql = buildTimeSeriesSQL(q)
    expect(sql).toContain('AVG(duration_us) AS value')
  })
  ...
  it('escapes SQL single quotes in filter values', () => {
    ...
    const sql = buildTimeSeriesSQL(q)
    expect(sql).toContain("service_name = 'it''s'")
  })
...
describe('buildSummarySQL', () => {
  ...
  it('builds count_distinct_traces summary', () => {
    ...
    expect(sql).toContain('COUNT(DISTINCT trace_id)')
  })
})
...
describe('serializeQuery / deserializeQuery', () => {
  it('round-trips a query', () => {
    ...
    expect(restored.measure).toBe('duration_us')
    expect(restored.timeRange).toBe('6h')
    expect(restored.interval).toBe('15m')
    expect(restored.filters).toHaveLength(1)
    expect(restored.filters[0].column).toBe('service_name')
  })
  ...
  it('handles missing params gracefully', () => {
    ...
    expect(restored.timeRange).toBe('1h')
  })
})
